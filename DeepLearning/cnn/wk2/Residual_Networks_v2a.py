...
def ResNet50(input_shape = (64, 64, 3), classes = 6):
    ...
    # Stage 3 (≈4 lines)
    f_shape = [64, 64, 256]
    X = convolutional_block(X, f = 3, filters = f_shape, stage = 3, block='a', s = 1)
    X = identity_block(X, 3, f_shape, stage=3, block='b')
    X = identity_block(X, 3, f_shape, stage=3, block='c')
    X = identity_block(X, 3, f_shape, stage=3, block='d')

    # Stage 4 (≈6 lines)
    f_shape = [256, 256, 1024]
    X = convolutional_block(X, f = 3, filters = [256, 256, 1024], stage = 4, block='a', s = 1)
    X = identity_block(X, 3, f_shape, stage=4, block='b')
    X = identity_block(X, 3, f_shape, stage=4, block='c')
    X = identity_block(X, 3, f_shape, stage=4, block='d')
    X = identity_block(X, 3, f_shape, stage=4, block='e')
    X = identity_block(X, 3, f_shape, stage=4, block='f')

    # Stage 5 (≈3 lines)
    f_shape = [512, 512, 2048]
    X = convolutional_block(X, f = 3, filters = f_shape, stage = 5, block='a', s = 1)
    X = identity_block(X, 3, f_shape, stage=5, block='b')
    X = identity_block(X, 3, f_shape, stage=5, block='c')
